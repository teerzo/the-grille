...
function SceneLighting({ onFadeComplete }) {
  ...
  useFrame((_, delta) => {
    ...
    if (ambientRef.current) ambientRef.current.intensity = (Math.PI / 2) * lightFactor
  ...
  return (
    <>
      ...
      <spotLight ref={spotRef} position={[10, 10, 10]} angle={0.15} penumbra={1} decay={0} intensity={Math.PI} />
      <pointLight ref={pointRef} position={[-10, -10, -10]} decay={0} intensity={Math.PI} />
    </>
  )
}
...
function TileSpacingGrid() {
  const grid = useMemo(() => {
    const helper = new GridHelper(TILE_SIZE * GRID_CELLS, GRID_CELLS, '#ff6b6b', '#6b7280')
...
function getWallTransform(wallPos, floorPositions) {
  ...
  return {
    position: [wallPos[0] + dirX * (TILE_SIZE / 2), 0, wallPos[2] + dirZ * (TILE_SIZE / 2)],
    yaw,
  }
}
 ...
 */
function getLaserWallInnerWorldPoint(layerRows, cols, rowCount, markerCol, markerRow, floorPositions, layerBaseY) {
  const beamY = layerBaseY + LASER_BEAM_Y_OFFSET
  const adj = findAdjacentWallCell(layerRows, cols, rowCount, markerCol, markerRow)
  if (!adj) {
...
function findMatchingEPair(ePairs, lPair) {
  if (lPair.kind === 'horizontal') {
    return ePairs.find(
      (p) => p.kind === 'horizontal' && p.colStart === lPair.colStart && p.colEnd === lPair.colEnd,
    )
  }
  return ePairs.find(
    (p) =>
...
function endpointsForPair(layerRows, cols, rowCount, floorPositions, layerBaseY, pair) {
  if (pair.kind === 'horizontal') {
    const { rowIndex, colStart, colEnd } = pair
    return {
      start: getLaserWallInnerWorldPoint(layerRows, cols, rowCount, colStart, rowIndex, floorPositions, layerBaseY),
      end: getLaserWallInnerWorldPoint(layerRows, cols, rowCount, colEnd, rowIndex, floorPositions, layerBaseY),
    }
  }
  const { colIndex, rowStart, rowEnd } = pair
  return {
    start: getLaserWallInnerWorldPoint(layerRows, cols, rowCount, colIndex, rowStart, floorPositions, layerBaseY),
    end: getLaserWallInnerWorldPoint(layerRows, cols, rowCount, colIndex, rowEnd, floorPositions, layerBaseY),
  }
}
 ...
 */
function collectLaserSegments(layerRows, cols, rowCount, layerBaseY, floorPositions) {
  if (!layerRows || layerRows.length === 0 || rowCount === 0 || cols === 0) return []
  ...
  for (const lPair of lPairs) {
    const { start: startL, end: endL } = endpointsForPair(layerRows, cols, rowCount, floorPositions, layerBaseY, lPair)
    const ePair = findMatchingEPair(ePairs, lPair)
    if (ePair) {
      const { start: startE, end: endE } = endpointsForPair(layerRows, cols, rowCount, floorPositions, layerBaseY, ePair)
...
function LevelGrid({ baseRows, upperWallLayers, roofRows, floorChar, wallChar, playerChar, roofChar, showDebug, roofHeight }) {
  ...
  const baseLaserSegments = useMemo(
    () => collectLaserSegments(baseRows, cols, rows, 0, floorPositions),
    [baseRows, cols, rows, floorPositions],
  )
  const upperLaserSegments = useMemo(() => {
    const out = []
    upperWallLayers.forEach((layerRows, layerIndex) => {
      const layerBaseY = UPPER_WALL_BASE_Y + layerIndex * WALL_HEIGHT
      out.push(...collectLaserSegments(layerRows, cols, rows, layerBaseY, floorPositions))
    })
    return out
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('active-camera-changed', onActiveCameraChanged)
  }, [])
  ...
  useFrame(({ camera }) => {
    ...
    const nextHovered = hits.length > 0 ? hits[0].object.userData.tileKey : null
  ...
  return (
    <>
      ...
      {baseLaserSegments.map(({ startL, endL, startE, endE }, index) => (
        <LaserObject key={`laser-base-${index}`} startL={startL} endL={endL} startE={startE} endE={endE} />
      ))}
      {upperLaserSegments.map(({ startL, endL, startE, endE }, index) => (
        <LaserObject key={`laser-upper-${index}`} startL={startL} endL={endL} startE={startE} endE={endE} />
      ))}
...
function Scene() {
  ...
  const playerSpawn = useMemo(() => findPlayerSpawn(baseRows, PLAYER_CHAR), [baseRows])
  ...
  return (
    <>
      <Physics key={resetVersion} debug={showPhysicsDebug}>
